...
const mapStatus = (status) => {
  ...
  switch (lowerStatus) {
    case 'pendente':
      return { label: "Aguardando Vaga", className: "bg-yellow-100 text-yellow-800" };
    case 'matriculado':
    case 'concluido':
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const pendingInscriptions = inscriptionsData.filter(insc => insc.status && insc.status.status.toLowerCase() === 'pendente').length;
  ...
  const inscriptionStatusChartData = useMemo(() => {
    ...
    const statusCounts = allInscriptionsData.reduce((acc, inscription) => {
      ...
      if (status === "pendente" || status === "concluido") displayStatus = "Aguardando Vaga/Concluído";
    ...
    const backgroundColors = labels.map(label => {
      switch (label.toLowerCase()) {
        case 'Aguardando Vaga/concluído': return 'rgba(255, 206, 86, 0.8)'; // Amarelo
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 bg-gray-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 mb-8">
        ...
        <StatCard title="Inscrições" value={stats.totalInscriptions} icon={<InscriptionIcon />} linkTo="/admin/registrations" isLoading={isLoadingStats} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white p-6 rounded-xl shadow-lg">
            ...
            {!isLoadingRecents && recentInscriptions.length === 0 && !error && <p className="text-gray-500 text-center py-4">Nenhuma inscrição recente.</p>}
                  ...
                  return (
                    <li key={insc.id} className="py-3.5 flex justify-between items-center hover:bg-gray-50 px-2 -mx-2 rounded-md transition-colors">
                      ...
                      <div className="text-right">
                        <span className={`px-2.5 py-1 inline-flex text-xs leading-tight font-semibold rounded-full ${className}`}>
                          {label}
                        </span>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-xl shadow-lg">
            ...
            <div className="space-y-3">
              <Link to="/admin/companies/new" className="block w-full text-center btn btn-secondary">Nova Empresa</Link>
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg">
            ...
            <div className="h-64 md:h-72 relative">
              {/* Gráfico de linhas */}
              {monthlyInscriptionsChartData && <Line data={monthlyInscriptionsChartData} options={lineChartOptions} />}
            </div>
          </div>
        </div>
      </div>

...
export default function AuthorizedUsersForm() {
    ...
    // filtra o select de acordo com a empresa logada (previnir erro de usuarios serem cadastrados em outras empresas)
    const rawUser = localStorage.getItem("user");
    const user = rawUser ? JSON.parse(rawUser) : null;
    const loggedCompany = user?.id ? String(user.id) : "";

    const filteredCompanies = companies.filter(
    (company) => String(company.id) === loggedCompany
    );
    ...
    useEffect(() => {
        ...
        navigate("/company/AuthorizedUsers");
        ...
        try {
            if (isEditing) {
                ...
                setMessageModalProps({
                    ...
                    success: true,
                    onClose: () => navigate("/company/AuthorizedUsers"),
                });
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4 sm:p-6 lg:p-8">
            <div className="max-w-3xl mx-auto">
                 <button
                    onClick={() => navigate('/company/AuthorizedUsers')}

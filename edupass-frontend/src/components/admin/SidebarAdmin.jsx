...
const ScholarshipHoldersIcon = () => <UserCheck className="w-5 h-5 mr-3 group-hover:text-white transition-colors" />;
...
const ClockIcon = () => <Clock  className="w-5 h-5 mr-3 group-hover:text-white transition-colors" />;
  ...
  return (
    <aside className="w-64 h-screen bg-slate-800 text-white flex flex-col fixed top-0 left-0 z-40 shadow-lg">
      ...
      <nav className="flex-grow px-3 py-4 space-y-1.5 overflow-y-auto"> {/*  */}
        ...
        <NavItem to="/admin/companies" icon={<CompanyIcon />}>Empresas</NavItem>
        ...
        <NavItem to="/admin/registrations" icon={<InscriptionsIcon />}>Inscrições</NavItem>
        <NavItem to="/admin/WaitList" icon={<ClockIcon />}>Aguardando Vaga</NavItem>
        <NavItem to="/admin/AuthorizedUsers" icon={<ShieldCheckIcon />}>Usuários Autorizados</NavItem>
      </nav>

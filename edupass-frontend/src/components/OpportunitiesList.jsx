...
export default function OpportunitiesList({ currentFilters }) {
  ...
  const fetchCursos = async () => {
    ...
    try {
      ...
      const cursosAtivos = cursos.filter(curso => curso.institutions?.status === true);
      const oportunidadesFormatadas = cursosAtivos.map((curso) => {
        ...
        return {
          ...
          originalPrice: curso.originalValue ? `R$ ${Number(curso.originalValue).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : null,
          discountedPrice: curso.discountValue ? `R$ ${Number(curso.discountValue).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : null,
      ...
      const oportunidadesFiltradas = filtrarOportunidades(oportunidadesFormatadas, currentFilters);
      const limite = 48;
